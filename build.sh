...
docker build --network=host -t rest-api-build-image --rm -f Dockerfile.build .

# create a directory for target .deb files
mkdir -p /tmp/target

# create .deb packages inside of the build container
docker run --network=host --rm=true -v /tmp/target:/src -v $(pwd):/src/rest -w /src/rest rest-api-build-image dpkg-buildpackage -us -uc
...
docker build --network=host -t rest-api-image-test_local --rm -f Dockerfile.test .
